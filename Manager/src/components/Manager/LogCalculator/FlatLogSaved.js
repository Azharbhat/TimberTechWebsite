...
const FlatLogSaved = ({ navigation, route }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const snapshot = await get(ref(database, `Mills/${key}/FlatLogCalculations`));
                if (snapshot.exists()) {
    ...
    const handleItemPress = (item) => {
        navigation.navigate('FlatLogSavedDetail', { data: item });
    };
                ...
                renderItem={({ item }) => (
                    <TouchableOpacity onPress={() => handleItemPress(item)}>
                        <View style={styles.item}>
                            ...
                            <Text style={styles.itemText}>TotalArea: {item.total}</Text>
                            <Text style={styles.itemText}>TotalPrice: {item.totalPrice}</Text>
